...
/// <summary>
/// macOS implementation of text output (auto-typing) using SharpHook
/// Requires Accessibility permissions to simulate keyboard input
/// User will be prompted to grant permissions on first use
/// </summary>
public class MacOSOutputProvider : IPlatformOutput
{
    private readonly EventSimulator _simulator;

    public MacOSOutputProvider()
    {
    ...
    public async Task TypeTextAsync(string text, int delayMs = 10)
    {
        ...
        {
            // If simulation fails, it's likely due to missing Accessibility permissions
            Console.Error.WriteLine($"Failed to simulate keyboard input on macOS: {ex.Message}");
            ...
            throw new InvalidOperationException(
                "Failed to type text. Ensure Accessibility permissions are granted in System Preferences.",
                ex
            );
        }
    }
